...
reg_lambda=1.0
neg_prior=True
tags="shift_grid,neg_adv,${obs_noise_model},${model_architecture},${test_dataset_a},${test_dataset_b}"
d=$(date +%Y-%m-%d-%H:%M:%S)
neg_prior_mean_coeff=10

for priors_means_same_coords in 1 2 3 4 5 6 7
do
    name_prefix="${d}_FINAL_neg_adv_bernoulli_1_means_same_coords_${priors_means_same_coords}"
