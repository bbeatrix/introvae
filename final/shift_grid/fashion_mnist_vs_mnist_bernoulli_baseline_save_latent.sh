...
reg_lambda=1.0
neg_prior=False
tags="save_latent,baseline,${neg_dataset},${train_dataset},${test_dataset_a},${test_dataset_b},${obs_noise_model},${model_architecture}"
d=$(date +%Y-%m-%d-%H:%M:%S)

for neg_prior_mean_coeff in 0
do
   name_prefix="${d}_FINAL_baseline_bernoulli_1_mean_coeff_${neg_prior_mean_coeff}"
